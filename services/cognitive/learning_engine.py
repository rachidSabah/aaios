...
class CognitiveLearningEngine:
    ...
    async def metrics(self) -> list[LearningMetric]:
        ...
        return [
            LearningMetric(name="total_experiences", value=total, unit="count",
                           explanation="Total executions analyzed"),
            LearningMetric(name="success_rate", value=successes / total, unit="ratio",
                           explanation=f"{successes}/{total} succeeded"),
            LearningMetric(name="avg_cost", value=sum(e.cost_usd for e in experiences) / total, unit="USD",
                           explanation="Average cost per execution"),
            LearningMetric(name="avg_latency", value=sum(e.latency_s for e in experiences) / total, unit="seconds",
                           explanation="Average execution latency"),
            LearningMetric(name="avg_confidence", value=sum(e.confidence_score for e in experiences) / total, unit="ratio",
                           explanation="Average confidence score"),
            LearningMetric(name="avg_risk", value=sum(e.risk_score for e in experiences) / total, unit="ratio",
