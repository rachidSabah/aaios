...
class DigitalTwinEngine:
    ...
    def build_snapshot(
        ...
        *,
        ...
        # Core kernel
        nodes.append(DigitalTwinNode(
            node_id="kernel", node_type="kernel", name="Kernel",
            status=_health("kernel")[1], health_score=_health("kernel")[0],
            properties={"uptime_s": metrics.uptime_s, "throughput_per_s": metrics.event_bus_throughput_per_s},
        ))
        # Event bus
        nodes.append(DigitalTwinNode(
            node_id="event_bus", node_type="event_bus", name="Event Bus",
            health_score=1.0,
        ...
        # Supervisor
        nodes.append(DigitalTwinNode(
            node_id="supervisor", node_type="supervisor", name="Supervisor",
            status=_health("supervisor")[1], health_score=_health("supervisor")[0],
        ))
        edges.append({"source": "kernel", "target": "supervisor", "type": "manages"})
        # Mission manager
        nodes.append(DigitalTwinNode(
            node_id="mission_manager", node_type="mission_manager", name="Mission Manager",
            status=_health("mission_manager")[1], health_score=_health("mission_manager")[0],
            properties={"active_missions": metrics.active_missions, "total_missions": metrics.total_missions},
        ))
        edges.append({"source": "supervisor", "target": "mission_manager", "type": "delegates_to"})
        # Agent registry
        nodes.append(DigitalTwinNode(
            node_id="agent_registry", node_type="agent_registry", name="Agent Registry",
            status=_health("agent_registry")[1], health_score=_health("agent_registry")[0],
            properties={"total_agents": metrics.total_agents, "active_agents": metrics.active_agents},
        ))
        edges.append({"source": "supervisor", "target": "agent_registry", "type": "dispatches_via"})
        # Model router
        nodes.append(DigitalTwinNode(
            node_id="model_router", node_type="model_router", name="Model Router",
            status=_health("model_router")[1], health_score=_health("model_router")[0],
        ...
        # Memory
        nodes.append(DigitalTwinNode(
            node_id="memory", node_type="memory", name="Memory Manager",
            status=_health("memory")[1], health_score=_health("memory")[0],
            properties={"usage_mb": metrics.memory_usage_mb},
        ))
        edges.append({"source": "kernel", "target": "memory", "type": "uses"})
        # Experience store
        nodes.append(DigitalTwinNode(
            node_id="experience_store", node_type="experience_store", name="Experience Store",
            properties={"total_experiences": metrics.total_experiences},
        ))
        edges.append({"source": "memory", "target": "experience_store", "type": "indexes"})
        # Task queue
        nodes.append(DigitalTwinNode(
            node_id="task_queue", node_type="queue", name="Task Queue",
            properties={"depth": metrics.queue_depth},
        ))
        edges.append({"source": "mission_manager", "target": "task_queue", "type": "submits_to"})
        # Budget
        nodes.append(DigitalTwinNode(
            node_id="budget", node_type="budget", name="Budget",
            status=_health("budget")[1], health_score=_health("budget")[0],
            properties={
        ...
        # API
        nodes.append(DigitalTwinNode(
            node_id="api", node_type="api", name="REST API",
            properties={"routes": 73},
        ))
        edges.append({"source": "api", "target": "mission_manager", "type": "exposes"})
        # Dashboard
        nodes.append(DigitalTwinNode(
            node_id="dashboard", node_type="dashboard", name="Dashboard",
        ))
