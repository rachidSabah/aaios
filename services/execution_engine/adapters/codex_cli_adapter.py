...
class CodexCliAdapter(BaseExecutionEngineAdapter):
    def __init__(self, config: EngineConfig | None = None) -> None:
        if config is None:
            config = EngineConfig(
                type=EngineType.CODEX_CLI,
                name="codex_cli",
    ...
    async def _on_execute(self, task: Any) -> Any:
        ...
        _log.info("Executing with Codex CLI", goal=goal[:100])
        if not self._process:
            return {"goal": goal, "result": "mock_codex_execution", "mock": True}
    ...
    async def _on_estimate_cost(self, task: Any) -> EngineCostEstimate:
        return EngineCostEstimate(
            ...
            breakdown={"per_token": 0.000010, "estimated_tokens": 1500},
        )
