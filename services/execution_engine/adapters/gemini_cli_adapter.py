...
class GeminiCliAdapter(BaseExecutionEngineAdapter):
    ...
    async def _on_execute(self, task: Any) -> Any:
        ...
        _log.info("Executing with Gemini CLI", goal=goal[:100])
        if not self._process:
            return {"goal": goal, "result": "mock_gemini_execution", "mock": True}
    ...
    async def _on_discover_capabilities(self) -> EngineCapabilities:
        return EngineCapabilities(
            ...
            features=["code.read", "code.write", "code.review", "test.run", "shell.execute"],
        )
    ...
    async def _on_estimate_cost(self, task: Any) -> EngineCostEstimate:
        return EngineCostEstimate(
            ...
            breakdown={"per_token": 0.000005, "estimated_tokens": 1500},
        )
