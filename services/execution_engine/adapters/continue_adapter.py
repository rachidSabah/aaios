...
class ContinueAdapter(BaseExecutionEngineAdapter):
    ...
    async def _on_execute(self, task: Any) -> Any:
        ...
        _log.info("Executing with Continue", goal=goal[:100])
        if not self._process:
            return {"goal": goal, "result": "mock_continue_execution", "mock": True}
    ...
    async def _on_estimate_cost(self, task: Any) -> EngineCostEstimate:
        return EngineCostEstimate(
            ...
            breakdown={"per_token": 0.000012, "estimated_tokens": 1500},
        )
