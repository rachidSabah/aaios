...
class OpenHandsAdapter(BaseExecutionEngineAdapter):
    ...
    def _detect_version() -> str:
        try:
            import subprocess
            for name in ("openhands", "openhands-cli"):
    ...
    async def _on_execute(self, task: Any) -> Any:
        ...
        _log.info("Executing with OpenHands", goal=goal[:100])
    ...
    async def _on_discover_capabilities(self) -> EngineCapabilities:
        return EngineCapabilities(
            ...
            features=["code.read", "code.write", "code.refactor", "code.review",
                       "test.run", "shell.execute", "browser.navigate", "web.search"],
        )
